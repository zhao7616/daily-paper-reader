...
motivation: 传统的基于帧或事件的跟踪器在处理螺旋桨等快速周期性运动时，因违反平滑运动假设而容易产生漂移或失效。
method: 采用全事件驱动架构，结合单应性反向映射、卡尔曼滤波相位估计及批量迭代位姿优化，实现联合跟踪与RPM估计。
result: 在包含52个旋转物体的新发布TQE数据集上，该方法实现了11.8倍于实时的处理速度和微秒级延迟，性能优于各类基准方法。
conclusion: HelixTrack为高速旋转物体的实时感知提供了高效可靠的解决方案，填补了该领域在算法和公开数据集上的空白。
---
...
## 速览
**TLDR**：针对无人机和旋转机械中快速周期性运动跟踪的挑战，本文提出HelixTrack，一种全事件驱动的螺旋桨类物体跟踪与转速（RPM）估计方法。该方法通过单应性变换将事件反向映射到转子平面，利用卡尔曼滤波维护相位，并通过耦合相位残差与几何信息优化位姿。此外，研究者发布了TQE数据集。实验表明，该方法在保持微秒级延迟的同时，处理速度达到实时性的11.8倍，性能优于现有基准。 \
**Motivation**：传统的基于帧或事件的跟踪器在处理螺旋桨等快速周期性运动时，因违反平滑运动假设而容易失效。 \
**Method**：采用单应性变换将图像平面事件映射至转子平面，结合卡尔曼滤波与批量迭代更新，实现相位与位姿的联合估计。 \
**Result**：在发布的TQE数据集上，该方法实现了11.8倍于实时的处理速度和微秒级延迟，性能显著优于各类基准方法。 \
**Conclusion**：HelixTrack为高速旋转物体的实时感知提供了高效且鲁棒的解决方案，填补了该领域在联合跟踪与转速估计上的空白。
